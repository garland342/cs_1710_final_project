document.addEventListener("DOMContentLoaded", () => {
  if (typeof d3 !== 'undefined') {
    ...
    // Create SVG with defs for clipPath
    const svg = container.append("svg")
      .attr("width", width)
      .attr("height", height);

    // Define a single clipPath for circular images (centered at origin)
    ...
    // Calculate positions in a grid layout (4 columns x 2 rows for 8 circles)
    const cols = 4;
    const rows = 2;
    ...
    // Store original transform for each group
    ...
    // Add circular images with clipPath
    circleGroups.each(function(d, i) {
      const g = d3.select(this);
      
      // Add circle background (for hover effect)
      g.append("circle")
        .attr("r", circleRadius)
      ...
      // Add image with circular clip - centered at origin (0,0)
      g.append("image")
        .attr("href", d.image)
        .attr("x", -circleRadius)
        .attr("y", -circleRadius)
    ...
    // Add hover effects - scale from center without moving
    circleGroups
      .on("mouseenter", function() {
        ...
        // Scale from center - use the stored x,y coordinates
...
function createSpiralVisualization(containerId, dataPath) {
  ...
  // Instruction text comes first (after title in HTML)
  ...
  // Toggles with more space after instruction
  ...
  // City name with more space after toggles
  ...
  const foodImages = {
    urban: {
      ...
      0.5: [
        'images/shoprite.jpg',
        'images/melrose market.jpg',
        'images/island market.jpg'
      ],
      1: [
        'images/safeway.jpg',
        'images/gazzali\'s.jpeg',
        'images/handy market.jpg'
      ]
    },
    rural: {
      2: [
        'images/subway logo.jpg',
        'images/arbys-logo.jpg',
        'images/dollar general.jpg'
      ],
      5: [
        'images/mcdonalds logo.jpg',
        'images/burger king.jpg',
        'images/baldwin kwik mart.jpg'
      ],
      10: [
        'images/walmart logo.jpg',
        'images/winn-dixie.jpg',
        'images/save-a-lot.jpg'
      ],
      20: [
        'images/sprouts farmer market.jpg',
        'images/shoprite.jpg', // Note: Using shoprite as placeholder for Rowe's Supermarket (image not provided)
        'images/whole foods logo.jpg'
      ]
    }
  };

  function getColorForDistance(distance, mode) {
    // Reversed color scheme: red is closest, green is farthest
    if (mode === 'urban') {
      if (distance === 0.1) return '#dc2626'; // Red - closest
      if (distance === 0.3) return '#ff8c00'; // Orange
      if (distance === 0.5) return '#5a9216'; // Light green
      if (distance === 1) return '#2d5016'; // Dark green - farthest
    } else {
      if (distance === 2) return '#dc2626'; // Red - closest
      if (distance === 5) return '#ff8c00'; // Orange
      if (distance === 10) return '#5a9216'; // Light green
      if (distance === 20) return '#2d5016'; // Dark green - farthest
    }
    return '#666';
  }
  ...
  function buildSpiralPoints() {
    ...
    // Rotate so terminating point points north (-π/2)
  ...
  function showFoodSidebar(event, distance, mode) {
    ...
    const hasNoFood = foods === null || (foods.length === 1 && foods[0] && foods[0].includes('nothing.jpg'));
    
    // Create popup below the spiral - append to container after vizWrap
    const popup = container.append('div')
      .attr('id', 'spiral-food-popup')
      .attr('class', 'spiral-food-popup')
      .style('width', '100%')
    ...
    header.append('h2')
      ...
      .style('margin-bottom', '5px')
      .style('font-weight', 'bold')
      .text(`Food options within a ${distance} mile${distance !== 1 ? 's' : ''} radius`);
    ...
    if (hasNoFood) {
      // Show no entry image and message
      const noFoodContainer = content.append('div')
        .style('text-align', 'center')
      ...
      // Use nothing.jpg if available, otherwise no-entry.jpg
      const noFoodImage = foods && foods.length > 0 && foods[0].includes('nothing.jpg') 
        ? 'images/nothing.jpg' 
        : 'images/no-entry.jpg';
      
      imgContainer.append('img')
        .attr('src', noFoodImage)
        .attr('alt', 'No entry')
      ...
      noFoodContainer.append('div')
        .style('font-size', '1.5rem')
        .style('font-weight', 'bold')
        .style('color', '#2c3e50')
        .style('margin-top', '1rem')
        .text('No convenience stores or fast food options');
    } else {
      // Show images in a grid with proper sizing to show all text
      const imageGrid = content.append('div')
        .style('display', 'grid')
        .style('grid-template-columns', 'repeat(auto-fit, minmax(250px, 1fr))')
        .style('gap', '1.5rem')
        .style('max-width', '100%');

      foods.forEach((imagePath, i) => {
  ...
  function hideFoodSidebar() {
    d3.select('#spiral-food-popup')
      .transition()
      .duration(200)
      .style('opacity', '0')
      .remove();
  ...
  function placeMarkers() {
    ...
    // All markers are circles on the spiral
    enter.append('circle')
    ...
    all.select('circle')
      ...
      .on('click', function(event, d) {
        ...
        // If clicking the same distance, toggle it off
        if (module.selectedDistance === d) {
          module.selectedDistance = null;
          d3.selectAll('.spiral-marker-g circle')
            .transition()
            .duration(200)
            .attr('r', 10)
            .attr('stroke-width', 2);
      ...
      .on('mouseleave', function() {
        if (module.selectedDistance === null) {
          ...
          const currentD = d3.select(this.parentNode).datum();
          if (module.selectedDistance === currentD) {
            d3.select(this)
              .transition()
              .duration(200)
              .attr('r', 14)
    ...
    d3.select('body').on('click.spiral', function(event) {
      if (!d3.select(event.target).closest('.spiral-marker-g').node() && 
          !d3.select(event.target).closest('#spiral-food-popup').node()) {
        module.selectedDistance = null;
        d3.selectAll('.spiral-marker-g circle')
          .transition()
          .duration(200)
  ...
  function switchMode(mode) {
    ...
    // Hide sidebar and reset selection when switching modes
    module.selectedDistance = null;
    hideFoodSidebar();
